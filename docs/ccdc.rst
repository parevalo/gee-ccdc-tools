...
for the entire globe. As of March 27, 2020, these results include
...
process them. A model run we have been using so far is the ``'v5'``
model, but others include the BRDF-corrected ``'brdf'``, and ``'x5'``.
Until the GEE team publishes an official public dataset, we can use the
``'v5'`` or ``'brdf'`` for testing purposes. Finally, if we want to look
...
.. code:: javascript
    ...
    var ccdc = ccdcCol.filterMetadata('system:index', 'starts_with', 'v5_')
...
.. code:: javascript
    ...
    // Obtain coefficients
    var coefs = utils.CCDC.get_multi_coefs(ccdImage, formattedDate, SELECT_BANDS, SELECT_COEFS, true, SEGS, 'after')
